...
{
	[JsonObject]
	[JsonConverter(typeof(ScheduleJsonConverter<IYearlySchedule, ITimeOfYear>))]
	...
	public class YearlySchedule : ScheduleBase, IYearlySchedule
	{
		...
		{
			return _times?.GetEnumerator() ?? Enumerable.Empty<ITimeOfYear>().GetEnumerator();
		}
		...
		public static implicit operator YearlySchedule(ITimeOfYear[] times) =>
			new YearlySchedule();
	}
